...
epaper_ui::GlobalFooterState BuildState()
{
    ...
    // Toggles the Todos screen's Active/Archived view -- no badge (a pending-transcription count
    // was wired here previously but was never actually shown, since show_folder was always
    // false; that meaning is gone now that the icon has a real, unrelated destination. Pending
    // transcriptions still surface via the status bar's kFile icon instead).
